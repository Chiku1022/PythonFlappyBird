...
# NOTE :- This game works best on Android Devices
#         Install 'Pydroid3' from PlayStore and this File
...
LIFT = 4#math.ceil(SCREENWIDTH / 135)    # speed of upward movement
GRAVITY = (SCREENWIDTH / 1542.8571428571)   # speed of downward movement
...
def welcomeScreen(backgroundImage, playerImage):
    ...
    y, up, down, movement = playery, True, False, 40
    # Banner co-ordinates 
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT or (event.type==KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()                
            elif event.type == FINGERDOWN or event.type==MOUSEBUTTONDOWN or (event.type==KEYDOWN and (event.key == K_SPACE or event.key == K_UP)):
    ...
    while(True):
        for event in pygame.event.get():
            if event.type == QUIT or (event.type==KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()
            if event.type == FINGERDOWN or event.type == MOUSEBUTTONDOWN or (event.type==KEYDOWN and (event.key == K_SPACE or event.key==K_UP)):
...
def gameover(score, backgroundImage, playerImage):
    ...
    while True:
        ...
        # Bird image display
        SCREEN.blit(playerImage, ((SCREENWIDTH-playerImage.get_width())//2, scoreboardHeight+scoreboardy*1.3))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit(0)
            elif event.type==FINGERDOWN or event.type==MOUSEBUTTONDOWN or (event.type==KEYDOWN and (event.key==K_UP or event.key==K_SPACE)): return
            else: pass
        FPSCLOCK.tick(FPS)
